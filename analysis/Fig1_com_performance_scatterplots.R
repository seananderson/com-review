...
plot_data <- function(obs, ests, xaxt, yaxt){
  ...
  if(xaxt==T){axis(1, at=0:5, labels=T, cex.axis=0.8, tck=tck)}else{axis(1, at=0:5, labels=F, tck=tck)}
  if(yaxt==T){axis(2, at=0:5, labels=T, cex.axis=0.8, tck=tck)}else{axis(2, at=0:5, labels=F, tck=tck)}
